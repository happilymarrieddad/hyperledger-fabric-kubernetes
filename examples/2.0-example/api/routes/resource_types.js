...
router.post('/', async (req, res) => {
    ...
    await contract.submitTransaction('create', newResourceType.id, JSON.stringify(newResourceType)).catch(err => res.status(400).send(err));;
...
router.put('/:id', async (req, res) => {
    ...
    if (req.body.name) {
        newResourceType.name = req.body.name
    }
    ...
    await contract.submitTransaction('update', req.params.id, JSON.stringify(newResourceType)).catch(err => res.status(400).send(err));;
